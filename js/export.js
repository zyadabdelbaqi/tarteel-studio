...
async function ensureFFmpegReady() {
    ...
    if (!ffmpegLibPromise) {
        ffmpegLibPromise = (async () => {
                ...
                // إضافة مسجل أحداث لمتابعة ما يحدث في الـ Console إذا لزم الأمر
                ffmpeg.on('log', ({ message }) => console.log('[FFmpeg]', message));

                // إجبار استخدام النسخة القياسية لتجنب مشاكل عدم توفر ملفات mt-worker
                // استخدام مسارات مطلقة لتعمل داخل الـ Blob Worker
                const baseHref = new URL('.', window.location.href).href;
                const corePrefix = new URL('ffmpeg/ffmpeg-core', baseHref).href;

                // استخدام Cache API لتحميل وتخزين ملفات FFmpeg مرة واحدة فقط لتسريع التصدير
                const fetchCleanScript = async (url, mimeType) => {
                    ...
                    text = text.replace(/import\.meta\.hot[^\n]*/g, '');
                
                // إضافة دالة وهمية لتجنب خطأ __vite__injectQuery الذي يحقنه Vite في بيئة التطوير
                text = "self.__vite__injectQuery = function(url) { return url; };\n" + text;
                
                    return URL.createObjectURL(new Blob([text], { type: mimeType }));
                };
                
                // تخزين ملف الـ WASM الثقيل في الكاش
                ...
                const workerBlobUrl = await fetchCleanScript('js/lib/814.ffmpeg.js', 'text/javascript');
                const coreBlobUrl = await fetchCleanScript(`${corePrefix}.js`, 'text/javascript');
                const wasmUrl = await fetchWasm(`${corePrefix}.wasm`);
...
function showProModal(type, data = {}) {
    ...
    const messages = {
        'audio': "استخدم صوتك الخاص مع الآيات بالترقية للنسخة الاحترافية.",
...
async function _secureExportImpl() {
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
        try {
            ...
            if (err && err.message && err.message.startsWith("WebCodecs Encoder fallback") && attempt < maxAttempts) {
    ...
    try {
        ...
        const processFrame = async (frameIdx) => {
            ...
            if (frameIdx % 3 === 0 || frameIdx === totalFrames - 1) {
                ...
                setExportProgress(progress); updateExportStatus(`الآية ${currentVerse} من ${totalVerses}`);
            }
        ...
        if (isSmallCase) {
            ...
            for (let j = startIdx; j < endIdx; j++) {
                ...
                if (state.audioCache[key]) {
                    ...
                    state.ayahs[j].exactDuration = Math.max(0.1, state.audioCache[key].duration - startSilence - endSilence);
            ...
            for (let i = startIdx; i < initialEndIdx; i++) {
                ...
                if (state.audioCache[key]) {
                    const { startSilence, endSilence } = calculateSilence(state.audioCache[key]);
                    state.ayahs[i].exactDuration = Math.max(0.1, state.audioCache[key].duration - startSilence - endSilence);
