...
export async function realExport(validatedCaps) {
    ...
    if (state.audioMode === 'local' && state.localAudioFile && !state.localAudioBuffer) {
        if (state.localAudioFile.size <= 15 * 1024 * 1024) {
            ...
            try {
                ...
                state.localAudioBuffer = await decodeCtx.decodeAudioData(arrayBuffer);
                if (!state.audioContext && decodeCtx && typeof decodeCtx.close === 'function') {
                    try { decodeCtx.close(); } catch(e) {}
                }
            } catch (e) {
                console.error("Local audio decode error:", e);
                throw new Error(EXPORT_ERRORS.AUDIO_FETCH);
            }
        }
    }
...
async function processOfflineAudio(offlineDuration, onlineVerseTimings, startIdx, endIdx, sampleRate, audioConfig) {
    ...
    const USE_CHUNKED_RENDERING = offlineDuration > 1800 || (state.audioMode === 'local' && state.localAudioFile && state.localAudioFile.size > 15 * 1024 * 1024); // استخدام النظام الآمن للملفات الأطول من 30 دقيقة أو الملفات الضخمة
    ...
    if (USE_CHUNKED_RENDERING) {
        ...
        let ffmpegInputWritten = false;
        const tmpInput = 'chunk_input_tmp.dat';
        ...
        for (let chunkIndex = 0; chunkIndex < totalChunks; chunkIndex++) {
            ...
            if (state.audioMode === 'local') {
                ...
                let isTrimmedChunk = false;

                if (!localBuf && state.localAudioFile) {
                    if (state.localAudioFile.size > 15 * 1024 * 1024) {
                        const { ffmpeg, fetchFile } = await ensureFFmpegReady();
                        if (!ffmpegInputWritten) {
                            await ffmpeg.writeFile(tmpInput, await fetchFile(state.localAudioFile));
                            ffmpegInputWritten = true;
                        }
                        const tmpOutput = `chunk_output_${chunkIndex}.wav`;
                        await ffmpeg.exec(['-i', tmpInput, '-ss', String(offset + chunkStart), '-t', String(renderDuration), '-ar', String(sampleRate), '-ac', '2', tmpOutput]);
                        const chunkWavData = await ffmpeg.readFile(tmpOutput);
                        const decodeCtx = state.audioContext || new (window.AudioContext || window.webkitAudioContext)();
                        localBuf = await decodeCtx.decodeAudioData(chunkWavData.buffer);
                        isTrimmedChunk = true;
                        try { await ffmpeg.deleteFile(tmpOutput); } catch(e) {}
        ...
        if (ffmpegInputWritten) {
            const { ffmpeg } = await ensureFFmpegReady();
            try { await ffmpeg.deleteFile(tmpInput); } catch(e) {}
        }
