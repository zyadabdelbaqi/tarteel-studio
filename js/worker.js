...
// الاعتماد الأساسي أصبح على الاستدعاء التفاعلي (Reactive) فور إضافة إطار، وهذا مجرد (Fallback) لزيادة الأمان
setInterval(monitorMemory, 10000);

function getSceneHash(exportTime = 0) {
    return [
        state.width, state.height,
        state.ayahText, state.translation,
        state.bgX, state.bgY, state.zoom, state.blur, state.overlayOpacity,
        state.fontSize, state.fontName, state.textY, state.textColor, state.shadowColor, state.shadowBlur,
        state.transTextColor, state.transShadowColor, state.transShadowBlur,
        state.animProgress,
        state.showWaveform ? currentAudioHash : 0, // 💡 الاعتماد على البصمة الفعلية للبيانات بدلاً من عداد التحديثات
        state.mediaType === 'video' ? exportTime : 0, // 💡 الاعتماد المباشر على توقيت حلقة التصدير (Export Loop Time) لضمان دقة لا متناهية
        state.showTranslation, state.showSurahName, state.surahY, state.surahX, state.surahFontSize,
        state.showWatermark, state.watermarkType, state.watermarkText, state.watermarkColor, state.watermarkSize, Math.round(state.watermarkOpacity * 100), state.watermarkX, state.watermarkY,
        state.showBasmala, state.basmalaNumber, state.basmalaX, state.basmalaY, state.basmalaSize, state.basmalaColor, state.basmalaShadowColor, state.basmalaShadowBlur
    ].join('|');
}
// -------------------------------------------------------
...
self.onmessage = async (e) => {
    ...
    if (type === 'init') {
        ...
        // نُنشئ البصمة. إذا كان الإطار "KeyFrame" نجبره على الريندر ليتمكن المرمز من وضع نقطة ارتكاز (I-Frame)
        const sceneHash = data.keyFrame ? `keyframe_${data.frameNumber}` : getSceneHash(data.timestamp);
        const frameDurationUs = Math.round(1_000_000 / FPS); // مدة الإطار الواحد بالمايكروثانية

        const isSameFrame = sceneHash === lastSceneHash;
...
function draw() {
    ...
    if (state.ayahText && ((fontName !== 'AlMushaf' && !fontName.startsWith('QuranPage')) || state.loadedFonts.has(fontName))) {
        ...
        ctx.translate(width/2, dynamicStartY + animTranslateY);
