...
import { initAudio, stopAudio, playSeamless, clearLocalAudioFile, updateAudioEffectParams, fetchAudioBuffer, addToAudioCache, clearAudioCache } from './audio.js';
import { getCache, setCache } from './db.js';
import { getUserCapabilities, getAllowedRange } from './permissions.js';
import { isExporting } from './exportQueue.js';
import { createIcons, icons } from 'lucide';

// توفير lucide بشكل عام ليتوافق مع الاستدعاءات الحالية في الكود
...
function decodeHTMLEntities(text) {
    ...
    async function start() {
        ...
        const checkCurrentSession = async () => {
            ...
            // منع تكرار الفحص بشكل مفرط (يجب مرور دقيقتين على الأقل منذ آخر فحص ناجح)
            if (Date.now() - lastSessionCheckTime < 120000) return;
        ...
        // الفحص الدوري كل 5 دقائق لتخفيف الطلبات (Requests) على الخوادم
        setInterval(checkCurrentSession, 300000);
            ...
            try {
                ...
                if (s.state.backgroundUrl && s.state.mediaType === 'image') {
                    ...
                    document.querySelectorAll('#stockImages img').forEach(img => {
                        const baseUrl = img.getAttribute('data-src') || img.src.split('?')[0];
        ...
        if (isExporting() || UI.limitModal.style.display === 'flex' || ['INPUT', 'TEXTAREA', 'SELECT'].includes(e.target.tagName) || e.target.isContentEditable) return;
                if (e.code === 'Space') {
                    ...
                    if (state.currentAyahIndex + 1 < state.ayahs.length) {
                        ...
                        if (caps.isFree && (state.currentAyahIndex + 1 - startIdx) >= 7) { UI.limitModal.style.display = 'flex'; if (state.isPlaying) stopAudio(); return; }
        ...
        if (lpThemeToggle) lpThemeToggle.onclick = toggleTheme;
            ...
            UI.playBtn.onclick = () => { if (state.isSyncing) return; if (state.isPlaying) stopAudio(); else { state.isPlaying = true; playSeamless((parseInt(UI.vStart.value) || 1) - 1); } };
        ...
        UI.confirmLimit.onclick = () => { UI.vEnd.value = (parseInt(UI.vStart.value) || 1) + 4; UI.limitModal.style.display = 'none'; updateDurationDisplay(); };
                ...
                if (UI.viewPricingBtn) {
                    UI.viewPricingBtn.onclick = () => {
                        ...
                        const video = lp.querySelector('video');
                        if (video) video.play().catch(e => console.warn(e));
                        setTimeout(() => {
                            lp.style.opacity = '1';
