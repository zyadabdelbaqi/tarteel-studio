    ...
    if (containerMonthly && !containerMonthly.hasChildNodes()) {
        paypal.Buttons({
            ...
            onApprove: async function(data, actions) {
                document.body.insertAdjacentHTML('beforeend', '<div id="paypal-loading-overlay" style="position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(0,0,0,0.8);z-index:99999;display:flex;flex-direction:column;align-items:center;justify-content:center;color:white;font-family:inherit;"><div style="border:4px solid #333;border-top:4px solid #007AFF;border-radius:50%;width:50px;height:50px;animation:spin 1s linear infinite;margin-bottom:20px;"></div><h2 style="font-size:24px;font-weight:bold;margin:0;">جاري تفعيل الاشتراك...</h2><p style="opacity:0.8;margin-top:10px;">يرجى الانتظار ثواني قليلة حتى يتم تأكيد الدفع</p></div><style>@keyframes spin { 0% { transform: rotate(0deg); } 100% { transform: rotate(360deg); } }</style>');
                const { data: { session } } = await supabaseClient.auth.getSession();
                if (session) {
                    for (let i = 0; i < 15; i++) {
                        const { data: profile } = await supabaseClient.from('profiles').select('plan').eq('id', session.user.id).maybeSingle();
