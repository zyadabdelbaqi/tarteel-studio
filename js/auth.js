...
export function updateProfileUI() {
    ...
    UI.profileName.innerText = name; UI.profileEmail.innerText = state.user.email;
    if (avatarUrl) {
        UI.profileAvatar.innerHTML = `<img src="${avatarUrl}" alt="${name}" class="w-full h-full rounded-full object-cover" referrerpolicy="no-referrer">`;
    } else {
        UI.profileAvatar.innerHTML = name.charAt(0).toUpperCase();
    }
...
export async function loadTestimonials() {
    ...
    try {
        ...
        if (data && data.length > 0) {
            UI.testimonialsGrid.innerHTML = data.map(t => `
                <div class="p-8 rounded-3xl bg-[var(--panel-bg)]/50 backdrop-blur-sm border border-[var(--border-color)] flex flex-col h-full hover:border-[#007AFF]/30 transition-colors">
                    <p class="text-sm text-zinc-400 mb-6 leading-relaxed flex-1">"${t.content}"</p>
                    <div class="flex items-center gap-3 mt-auto">
                        ${t.avatar_url ? `<img class="w-10 h-10 rounded-full bg-zinc-700 object-cover border border-[var(--border-color)]" src="${t.avatar_url}" alt="${t.name}">` : `<div class="w-10 h-10 rounded-full bg-gradient-to-tr from-[#007AFF] to-cyan-400 flex items-center justify-center text-white font-bold text-sm shadow-md">${t.name.charAt(0).toUpperCase()}</div>`}
                        <div><h4 class="font-bold text-sm text-[var(--text-main)]">${t.name}</h4></div>
                    </div>
                </div>
            `).join('');
        } else {
